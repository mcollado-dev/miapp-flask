pipeline {
    ...
    environment {
        ...
        SONAR_AUTH_TOKEN = credentials('sonar-token') // Token configurado en Jenkins
    }
                    ...
                    ssh -o StrictHostKeyChecking=no ${DEPLOY_USER}@${DEPLOY_HOST} '
                        ...
                        until curl -s -o /dev/null -w "%{http_code}" http://localhost:${APP_PORT} | grep 200 > /dev/null; do
                            ...
                            if [ \$COUNT -ge \$MAX_TRIES ]; then
                                echo "Flask no responde después de 30s"
                                exit 1
                            fi
                        done
    ...
    post {
        success { echo 'Pipeline completado: Flask desplegada y SonarQube OK.' }
